...
void print_usage(void) {
	...
	printf("-m		include asm files as input\n");//vimarsh
}
...
void handle_opts(int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv, "d:o:sgpefam")) != -1)
		switch (c) {
			case 'd':
				dvalue = optarg;
				break;
	...
	if (optind < argc) {
		if(minputs == 1){//vimarsh
			char str[5] = ".c";
			
			while (optind < (argc -1)) {
				if(strcmp(argv[optind],str)>0){
					numofasmfiles = argc - 1 - optind;
					ivalue = argv[optind];
					printf("input file:%s\nnumberofasmfiles = %d\n",ivalue,numofasmfiles);
					printf("vijay bhava : %d\n",optind);
...
int main(int argc, char *argv[]) {
	...
	if (NO_COMPILE == 0) {
		...
		if(numofasmfiles > 0){//vimarsh
			char* jumptoend = "j end\nnop\n";
			fprintf(FILE_OUTPUT, "%s", jumptoend);
			int lastfile = (argc - numofasmfiles);
			while(numofasmfiles != 0){
